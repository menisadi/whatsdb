#!/usr/bin/env python3
"""Plot WhatsApp message frequency over time."""
...
def per_period(
    ...
    ax.plot(counts.index, counts.values, alpha=0.4, label="raw")
    if smooth is not None:
        smoothed = counts.rolling(smooth, min_periods=1).mean()
        ax.plot(smoothed.index, smoothed.values, label=f"{smooth}-period avg")
        ax.legend()
...
def by_sender(
    ...
    pivoted.columns = [get_display(c) for c in pivoted.columns]

    _, ax = plt.subplots(figsize=(14, 5))
    pivoted.plot(ax=ax, kind="area" if stacked else "line", stacked=stacked)
