...
_MSG_RE = re.compile(
    r"^(?:"
    + r"\[(\d{2}/\d{2}/\d{4}, \d{1,2}:\d{2}:\d{2})\] "  # new: [DD/MM/YYYY, HH:MM:SS] …
    + r"|"
    + r"(\d{2}/\d{2}/\d{4}, \d{1,2}:\d{2}) - "  # old: DD/MM/YYYY, HH:MM - …
    + r")(.*)"
)
...
def _build_db(db_path: Path, messages: list[Message]) -> None:
    ...
    cur.executescript("""
        ...
        CREATE INDEX idx_dedup  ON messages(STRFTIME('%Y-%m-%d %H:%M', ts), body, sender);
